...
local starboy = display.newRect(40, 200, 20, 20)
starboy:setFillColor(0.5,0.7,0.9)

physics.addBody(starboy)
starboy.gravityScale = 0
starboy.isFixedRotation = false
starboy.isSensor = true
...
local function onUpdate (event)
  ...
  if(moveUp > 0) then
    starboy.y = starboy.y - moveUp
    moveUp = moveUp - 0.8
  end
  starboy.y = starboy.y + moveDown

  if(starboy.y < 0) then
    endGame()
  elseif(starboy.y > 520) then
    endGame()
  end
end
...
local function onLocalCollision (self, event)
  if(event.phase == "began") then
    if(self.ID == "starboy" and event.other.ID == "Crash") then
      endGame()
    end
  end
end
...
function endGame ()
  starboy:removeEventListener("collision", starboy)
...
starboy.collision = onLocalCollision
starboy:addEventListener("collision", starboy)

Runtime:addEventListener("enterFrame", onUpdate)
background:addEventListener("touch", flapBoy)